...
%bt ... "were more difficult"  die Aufgabe war schwieriger zu bewältigen? woran zeigte sich das? evtl. hier kurz die AV erwähnen
...
%bt Allgemeine Anmerkung: Ich hatte den Eindruck, dass die Begriffe displacement und motion manchmal in verschiedenen Bedeutungen verwendet werden. Displacement nennst Du die Verschiebung des Rotationspunkts. Wenn ich das alles richtig verstanden habe gehts darum, dass diese Verschiebung dazu führt, dass die Rotation dann als Bewegung (motion) wahrgenommen wird, richtig? Müsste dann in dem SAtz nicht stehen ...the manipulated orientation change has to be perceived as an orientation change and not as motion? Vielleicht habe ichs aber auch nur noch nicht ganz kapiert...
This is illustrated in figure \ref{fig:rot} on the example of darts used by \cite{gao10}. 
%bt Anmerkung zur Fugure 2: Evtl. den grünen Punkt nicht grün anzeigen, da man sonst meinen könnte, dies wäre der grüne Circle, den die Vpn steuert (zugegeben, das wären dumme Leser, aber man weiß ja nie...) 
...
%bt das klingt ein wenig so, als ob wir zwingend eine solche ALternativerklärung annehmen müssen... habe aber auch keine bessere Formulierung.
...
%bt habe wolf rausgenommen, um die Bezeichnungen der Stimuli möglichst konstant zuhalte - macht es für den Leser einfacher.
...
%bt dieser Abschnitt (oben ab ---on the other hand) ist schweirig nachzuvollziehen. Evtl. kann man dieses Arguemtn besser verstehen nachdem die Aufgaben beschrieben wurden.
...
%bt dieser Satz sollte auch unbedingt im Abstract so ähnlich auftauchen, da er die Hauptpunkte der Studien super zusammenfasst und den Mehrgewinn gegenüber den bisherigen Arbeiten darelgt.
...
%bt Halte ich in der Einführung nicht für zwingend - würde nur verwirren.
...
% bt stimmt, damit erledigt sich meine Anmerkung oben.
...
%bt Altersangaben sollten noch rein (Mittleres alter und Range). Waren das wirklich nur 7 männliche Teilnehmer?
...
%bt sind es 3 Blocks oder 4 Blocks? Wenn in jedem Block 42 trials enthalten sind, dann müssten es 4 Blocks sein, sonst gibst Probleme an der Stelle wo Du in Block 3 die 18 und die 24 trials beschreibst und danach noch die 13 letzten trials. Du kannst von 3 Hauptblöcken schreiben und die letzten 13 trials als final oder addtional block einführen, dann musst Du nicht schon vorher detailliert darauf eingehen.
...
%bt evtl. umstellen. Erst beschreiben, wie die Vpn sitzt und dann auf die Instruktionen eingehen.
...
\section*{Results}
%bt ich habe jetzt nicht ganz aufgepasst. Bleibst Du bei der Beschreibung der REsults in der Reihenfolge wie die Aufgaben auch gestellt wurden? Wenn nicht wäre es gut, einen Satz zu schreiben, warum Du welche Ergebnisse vorziehst.
...
\begin{figure}[!ht]
\begin{center}
\includegraphics[width=3.27in]{fig/rot}
\end{center}
\caption{
...
\begin{figure}[!ht]
\begin{center}
\includegraphics[width=4.86in]{fig/compar}
\end{center}
\caption{
